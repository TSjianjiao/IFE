 ...
 */
drawLineChart.prototype.drawXY = function (inputData) {
    // 拿到折线图中的最大值Max
    // 保存数据 将所选数据平铺
    var data = [];
    ...
    while(maxData > YLabel) {
        ...
        context.fillText(YLabel, this.dataSpace - 35, this.axisHeight - YLabel * scale + 5);
        // 画网格
        context.moveTo(this.dataSpace, this.axisHeight - YLabel * scale);
...
drawLineChart.prototype.drawLineByCanvas = function (inputData) {
    ...
    // 绘制横纵轴以及纵轴标签 返回尺度
    var scale = this.drawXY(inputData);
    // 遍历画出每选择的折线图
    // 记录数据数量
...
class drawBarChart {
    ...
    // 不同聚合方式绘制柱状图
    drawBarByGather (className, dataLabelSpace, inputData, scale) {
        // 聚合的柱体数量
        var barNum;
        var productNum = 0;
        var temp = '';
    ...
    drawMultiBarBySvg (inputData) {
        ...
        // 如果是按照产品聚合的
        if (flag) {
            this.drawBarByGather('product', 55, inputData, scale);
        }
        // 按照地区聚合 和按照产品聚合原理一样
        else {
            this.drawBarByGather('region', 20, inputData, scale);
        }
    }
}