 ...
 * 通过hash记录用户操作
 */
function recordOpByHash() {
    var selectedCheck = [];
    [...document.querySelectorAll('input[check-type="option"]')].forEach(item=>{
        if(item.checked) selectedCheck.push(item.id)
    });
    location.hash = '#' + selectedCheck.join('');
}

/**
 * 通过history记录用户操作
 */
function recordOpByHistory() {
    var selectedCheck = [];
    ...
    var state = { 'id': selectedCheck};
    var url = `#${selectedCheck.join('')}`
